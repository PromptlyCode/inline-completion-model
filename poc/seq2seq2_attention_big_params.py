...
class Encoder(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size, n_layers=4, dropout=0.2):
    ...
    def forward(self, src):
        ...
        # Enhanced hidden state processing
        hidden_cat = torch.cat((hidden[-2,:,:], hidden[-1,:,:]), dim=1)
        cell_cat = torch.cat((cell[-2,:,:], cell[-1,:,:]), dim=1)
        
        hidden = self.fc(hidden_cat)
        cell = self.fc(cell_cat)
...
class Attention(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        energy = self.attention_combine(torch.cat((hidden, encoder_outputs), dim=2))
        v = self.v.repeat(batch_size, 1).unsqueeze(1)
        
        attention = torch.bmm(energy, v.transpose(1, 2)).squeeze(2)
        attention = attention / self.scale.to(attention.device)
        
        return torch.softmax(attention, dim=1)

class Decoder(nn.Module):
    def __init__(self, output_size, embed_size, hidden_size, n_layers=4, dropout=0.2):
